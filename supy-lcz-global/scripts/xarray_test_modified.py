...
split_site_list_df = pd.read_csv(Path('./resources/GreaterKL-2017_Y1_M2sp_3sf_R4_splitlist.csv'), index_col = 'sitename')
#split_site_list_df.index.name = 'sitename'

site_prefix = r'GreaterKL-2017_Y1_M2sp_3sf_R4'
site_midpoint_lat = 3.056577
site_midpoint_lon = 101.617373

output_file = r'./data/consolidated_outputs/'
variable_list = ['Kdown', 'Kup', 'Ldown', 'Lup', 'Tsurf', 'QN', 'QF', 'QS', 'QH', 'QE', 'AlbBulk', 'Ts', 'T2', 'Q2', 'U10', 'RH2']
...
print(f"Coordinate arrays: {lat_2d.shape}, {lon_2d.shape}")
print(f"First few coords - Lat: {lat_2d[0,1]:.6f}, Lon: {lon_2d[0,1]:.6f}")

print("First 10 coordinates:")
for i in range(10):
    print(f"Grid {i}: {grid_lat_list[i]:.6f}, {grid_lon_list[i]:.6f}")
